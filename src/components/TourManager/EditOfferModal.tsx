...
type Props = {
  offer: TourOfferDto
  onClose: () => void
  onSave: (updated: TourOfferDto) => void
}

export default function EditOfferModal({ offer, onClose, onSave }: Props) {
  const [edited, setEdited] = useState<TourOfferDto>({
    ...offer,
    countryId: offer.countryId || 0,
    city: offer.city || ''
  })
  ...
  // Завантажуємо список країн один раз
  useEffect(() => {
    axios
  ...
  // При першому завантаженні одразу підтягуємо список міст
  useEffect(() => {
  ...
  // Коли змінюється країна — підтягуємо її міста
  useEffect(() => {
    if (!edited.countryId || edited.countryId <= 0) {
      setCities([])
  ...
  const handleSaveClick = async () => {
    setSaving(true)
    try {
      const res = await axios.put<TourOfferDto>(
        `https://localhost:7181/api/tour-manager/offers/${edited.baseTourId}`,
        edited
      )
      onSave(res.data)
      onClose()
    } catch (err) {
      console.error('Помилка збереження пропозиції:', err)
    ...
    <div className="fixed inset-0 z-50 flex justify-center items-center bg-black bg-opacity-40">
      <div className="bg-white rounded-xl p-6 w-full max-w-2xl shadow-lg overflow-y-auto max-h-[90vh]">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-primary">Редагувати пропозицію</h2>
